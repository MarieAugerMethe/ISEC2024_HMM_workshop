...
This section will illustrate how to model mean step length as a function of a different covariate: time of day. 
...
$$\mu_{i}^{(t)} = \beta^{(i)}_0 + \beta_1^{(i)} \cos \left(\frac{2 \pi \tau_t}{24}\right) +  \beta_2^{(i)} \sin \left(\frac{2 \pi \tau_t}{24}\right) $$
...
To do link the formula to the mean step length, we specify `DM`, which will be a list of formulas for the step length (and possibly other) parameters. 
