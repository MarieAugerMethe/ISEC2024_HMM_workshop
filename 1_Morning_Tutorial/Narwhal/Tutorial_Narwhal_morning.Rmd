...
tracks_gps$tod <- hour(tracks_gps$time) + minute(data$time) / 60
```
...
<p style='text-align: justify;'> 
Our data is ready to use, and are almost ready to fit the HMM (via
...
As the observation data is the same between these models and the model in section \@ref(void-gaps), we can use AIC to compare models with and without time of day effects.
